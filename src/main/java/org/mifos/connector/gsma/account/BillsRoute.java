...
import static org.mifos.connector.gsma.camel.config.CamelProperties.BILLS;
import static org.mifos.connector.gsma.camel.config.CamelProperties.BILLS_ACTION;
import static org.mifos.connector.gsma.camel.config.CamelProperties.BILLS_CALL_FAILED;
import static org.mifos.connector.gsma.camel.config.CamelProperties.BILLS_REQUEST_BODY;
import static org.mifos.connector.gsma.camel.config.CamelProperties.BILL_COMPANIES;
import static org.mifos.connector.gsma.camel.config.CamelProperties.BILL_REFERENCE;
...
public class BillsRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
         ...
         */
        from("direct:bills-route-base").id("bills-route-base")
                ...
                .log(LoggingLevel.INFO, "Got access token, moving on to API call.").choice()
                .when(exchange -> exchange.getProperty(BILLS_ACTION, String.class).equals("companies")).to("direct:get-bills-companies")
                .log(LoggingLevel.INFO, "Bill Companies API response: ${body}").choice()
                .when(header("CamelHttpResponseCode").isEqualTo("200")).process(exchange -> {
                    exchange.setProperty(BILLS_CALL_FAILED, false);
                    exchange.setProperty(BILL_COMPANIES, exchange.getIn().getBody(String.class));
                }).otherwise().to("direct:bills-error-handler").endChoice()
                .when(exchange -> exchange.getProperty(BILLS_ACTION, String.class).equals("bills")).to("direct:get-bills")
                .log(LoggingLevel.INFO, "Bills API response: ${body}").choice().when(header("CamelHttpResponseCode").isEqualTo("200"))
                .process(exchange -> {
                    exchange.setProperty(BILLS_CALL_FAILED, false);
                    exchange.setProperty(BILLS, exchange.getIn().getBody(String.class));
                }).otherwise().to("direct:bills-error-handler").endChoice()
                .when(exchange -> exchange.getProperty(BILLS_ACTION, String.class).equals("payment")).to("direct:bills-payment")
                .log(LoggingLevel.INFO, "Transaction API response: ${body}").to("direct:transaction-response-handler").otherwise()
                .log(LoggingLevel.INFO, "No suitable bill action found").end();

        /**
         * Bills error handler
         */
        from("direct:bills-error-handler").id("bills-error-handler").log(LoggingLevel.INFO, "Error in bills request").unmarshal()
         ...
         */
        from("direct:get-bills-companies").id("get-bills-companies").removeHeader("*").setHeader(Exchange.HTTP_METHOD, constant("GET"))
                ...
                .toD(baseURL + account + "/${exchangeProperty." + IDENTIFIER_TYPE + "}/${exchangeProperty." + IDENTIFIER + "}"
                        + "/billcompanies" + "?bridgeEndpoint=true&throwExceptionOnFailure=false");

        /**
         * API call to get bills for an account
         */
        from("direct:get-bills").id("get-bills").removeHeader("*").setHeader(Exchange.HTTP_METHOD, constant("GET"))
                .setHeader("X-Date", simple(ZonedDateTime.now(ZoneOffset.UTC).format(DateTimeFormatter.ISO_INSTANT)))
                .setHeader("Authorization", simple("Bearer ${exchangeProperty." + ACCESS_TOKEN + "}"))
                .toD(baseURL + account + "/${exchangeProperty." + IDENTIFIER_TYPE + "}/${exchangeProperty." + IDENTIFIER + "}" + "/bills"
                        + "?bridgeEndpoint=true&throwExceptionOnFailure=false");

        /**
         * API call to initiate payment for bill
         */
        from("direct:bills-payment").id("bills-payment").removeHeader("*").setHeader(Exchange.HTTP_METHOD, constant("POST"))
                .setHeader("X-Date", simple(ZonedDateTime.now(ZoneOffset.UTC).format(DateTimeFormatter.ISO_INSTANT)))
                .setHeader("Authorization", simple("Bearer ${exchangeProperty." + ACCESS_TOKEN + "}"))
                ...
                .setHeader("Content-Type", constant("application/json")).setBody(exchange -> exchange.getProperty(BILLS_REQUEST_BODY))
                .marshal().json(JsonLibrary.Jackson).log(LoggingLevel.INFO, "Links Request Body: ${body}")
                .toD(baseURL + account + "/${exchangeProperty." + IDENTIFIER_TYPE + "}/${exchangeProperty." + IDENTIFIER
                        + "}/bills/${exchangeProperty." + BILL_REFERENCE + "}/payments?bridgeEndpoint=true&throwExceptionOnFailure=false");

        /**
         * Callback for bill payment
         */
        from("rest:PUT:/bills/payment/callback").log(LoggingLevel.INFO, "Callback body ${body}").unmarshal()
                ...
                .setProperty(TRANSACTION_FAILED, constant(false)).otherwise()
                .process(exchange -> exchange.setProperty(ERROR_INFORMATION,
                        exchange.getIn().getBody(RequestStateDTO.class).getError().getErrorDescription()))
                .setProperty(TRANSACTION_FAILED, constant(true)).end().process(transferResponseProcessor);

        /**
         * API to initiate bill payment
         */
        from("rest:POST:/bills/payment").log(LoggingLevel.INFO, "Got bill payment POST request").process(exchange -> {
