...
public class DebitMandateRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
         ...
         */
        from("direct:create-debit-mandate").id("create-debit-mandate").removeHeader("*").setHeader(Exchange.HTTP_METHOD, constant("POST"))
                ...
                .setHeader("Content-Type", constant("application/json")).setBody(exchange -> exchange.getProperty(DEBIT_MANDATE_BODY))
                .marshal().json(JsonLibrary.Jackson).log(LoggingLevel.INFO, "Debit Mandate Request Body: ${body}")
                .toD(baseURL + account + "/${exchangeProperty." + IDENTIFIER_TYPE + "}/${exchangeProperty." + IDENTIFIER
                        + "}/debitmandates?bridgeEndpoint=true&throwExceptionOnFailure=false");
        ...
        from("rest:POST:/account/debitmandate/{identifierType}/{identifier}").log(LoggingLevel.INFO, "Creating Debit Mandate")
                ...
                }).to("direct:debit-mandate-base").setBody(simple("${exchangeProperty." + MANDATE_REFERENCE + "}"));
