...
public class LinksRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
         ...
         */
        from("direct:links-route").id("links-route")
                .log(LoggingLevel.INFO, "Creating ${exchangeProperty." + ACCOUNT_ACTION + "} for Identifier").to("direct:get-access-token")
                .process(exchange -> exchange.setProperty(ACCESS_TOKEN, accessTokenStore.getAccessToken()))
                .log(LoggingLevel.INFO, "Got access token, moving on to API call.").to("direct:create-link-async")
                .log(LoggingLevel.INFO, "Transaction API response: ${body}").choice().when(header("CamelHttpResponseCode").isEqualTo("202"))
                .log(LoggingLevel.INFO, "Link request successful").unmarshal().json(JsonLibrary.Jackson, RequestStateDTO.class)
                .process(exchange -> {
                ...
                }).otherwise().log(LoggingLevel.ERROR, "Link request unsuccessful").unmarshal().json(JsonLibrary.Jackson, ErrorDTO.class)
                .process(exchange -> {
                ...
                }).process(linksResponseProcessor).endChoice();

        /**
         * Async call to Links API
         */
        from("direct:create-link-async").id("create-link-async").removeHeader("*").setHeader(Exchange.HTTP_METHOD, constant("POST"))
                ...
                .setHeader("Content-Type", constant("application/json")).setBody(exchange -> exchange.getProperty(LINKS_REQUEST_BODY))
                ...
                .toD(baseURL + account + "/${exchangeProperty." + IDENTIFIER_TYPE + "}/${exchangeProperty." + IDENTIFIER
                        + "}/links?bridgeEndpoint=true&throwExceptionOnFailure=false");

        /**
         * Sync call to Links API
         */
        from("direct:create-link").id("create-link").removeHeader("*").setHeader(Exchange.HTTP_METHOD, constant("POST"))
                .setHeader("X-Date", simple(ZonedDateTime.now(ZoneOffset.UTC).format(DateTimeFormatter.ISO_INSTANT)))
                .setHeader("Authorization", simple("Bearer ${exchangeProperty." + ACCESS_TOKEN + "}"))
                .setHeader("Content-Type", constant("application/json")).setBody(exchange -> exchange.getProperty(LINKS_REQUEST_BODY))
                .marshal().json(JsonLibrary.Jackson).log(LoggingLevel.INFO, "Links Request Body: ${body}")
         ...
         * Links API callback
         */
        from("rest:PUT:/links/callback").log(LoggingLevel.INFO, "Callback body ${body}").unmarshal()
                ...
                .setProperty(LINK_CREATE_FAILED, constant(false)).setProperty(RESPONSE_OBJECT_TYPE, constant("links"))
                ...
                }).log(LoggingLevel.INFO, "Link reference is: ${exchangeProperty." + LINK_REFERENCE + "}").otherwise()
                .setProperty(ERROR_INFORMATION, simple("Error in getting link reference.")).setProperty(LINK_CREATE_FAILED, constant(true))
                .endChoice().otherwise()
                ...
                .setProperty(LINK_CREATE_FAILED, constant(true)).end().process(linksResponseProcessor);

        /**
         * API to start base route
         */
        from("rest:POST:/links").log(LoggingLevel.INFO, "Got Links POST request").process(exchange -> {
