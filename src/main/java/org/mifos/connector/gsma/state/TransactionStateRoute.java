...
public class TransactionStateRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
         ...
         */
        from("direct:transaction-state-check").id("transaction-state-check").to("direct:transaction-state").process(exchange -> {
        ...
        from("direct:transaction-state").id("transaction-state").log(LoggingLevel.INFO, "Transaction State route started").choice()
                ...
                .log(LoggingLevel.INFO, "Getting Server Correlation ID").process(exchange -> {
                    Stream<String> serverCorrelations = correlationIDStore
                            .getServerCorrelations(exchange.getProperty(CORRELATION_ID, String.class));
                ...
                }).to("direct:get-transaction-state-channel").log(LoggingLevel.INFO, "Transaction State API response: ${body}").choice()
                .when(header("CamelHttpResponseCode").isEqualTo("200")).to("direct:transaction-state-success").otherwise()
                .log(LoggingLevel.INFO, "Transaction State Request Unsuccessful").to("direct:transaction-state-error").endChoice()
                .otherwise().log(LoggingLevel.INFO, "No Server Correlation found for Client").to("direct:transaction-state-error")
                .endChoice();

        /**
         * Route to call Transaction State API
         */
        from("direct:get-transaction-state").id("get-transaction-state").removeHeader("*").setHeader(Exchange.HTTP_METHOD, constant("GET"))
                .setHeader("X-Date", simple(ZonedDateTime.now(ZoneOffset.UTC).format(DateTimeFormatter.ISO_INSTANT)))
                .setHeader("Authorization", simple("Bearer ${exchangeProperty." + ACCESS_TOKEN + "}")).toD(baseURL + "/requeststates"
                        + "/${exchangeProperty." + SERVER_CORRELATION + "}" + "?bridgeEndpoint=true&throwExceptionOnFailure=false");

        from("direct:get-transaction-state-channel").id("get-transaction-state-channel").removeHeader("*")
                .setHeader(Exchange.HTTP_METHOD, constant("GET"))
                .setHeader("Platform-TenantId", simple("${exchangeProperty." + RECEIVING_TENANT + "}"))
                .toD(channelURL + "/channel/requeststates" + "/${exchangeProperty." + SERVER_CORRELATION + "}"
         ...
         */
        from("direct:transaction-state-success").id("transaction-state-success")
                .log(LoggingLevel.INFO, "Transaction State request successful").unmarshal().json(JsonLibrary.Jackson, RequestStateDTO.class)
                .process(exchange -> {
