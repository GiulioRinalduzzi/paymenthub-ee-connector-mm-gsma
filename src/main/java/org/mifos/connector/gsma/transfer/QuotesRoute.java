...
import static org.mifos.connector.gsma.camel.config.CamelProperties.QUOTE_REQUEST_BODY;
import static org.mifos.connector.gsma.camel.config.CamelProperties.QUOTE_RESPONSE;
...
public class QuotesRoute extends RouteBuilder {
    ...
    public void configure() {
        ...
        from("direct:quote-route-base").id("quote-route-base").log(LoggingLevel.INFO, "Transfer route started")
                .to("direct:get-access-token").process(exchange -> exchange.setProperty(ACCESS_TOKEN, accessTokenStore.getAccessToken()))
                .log(LoggingLevel.INFO, "Got access token, moving on").process(exchange -> {
                    QuotesDTO gsmaQuoteRequestBody = new QuotesDTO();
                    GSMATransaction gsmaChannelRequestBody = objectMapper
                            .readValue(exchange.getProperty(GSMA_CHANNEL_REQUEST, String.class), GSMATransaction.class);
                ...
                }).log(LoggingLevel.INFO, "Moving on to API call").to("direct:get-quote")
                .log(LoggingLevel.INFO, "Quote API response: ${body}").setProperty(QUOTE_RESPONSE, simple("${body}")).choice()
                .when(header("CamelHttpResponseCode").isEqualTo("201")).log(LoggingLevel.INFO, "Quote request successful").unmarshal()
                .json(JsonLibrary.Jackson, QuotesDTO.class).setProperty(GSMA_QUOTE_FAILED, constant(false)).process(exchange -> {
                ...
                }).otherwise().log(LoggingLevel.ERROR, "Quote request unsuccessful").unmarshal().json(JsonLibrary.Jackson, ErrorDTO.class)
                .process(exchange -> {
                    exchange.setProperty(ERROR_INFORMATION, exchange.getIn().getBody(ErrorDTO.class).getErrorCode());
                }).setProperty(GSMA_QUOTE_FAILED, constant(true)).end().process(quoteResponseProcessor);

        from("direct:get-quote").id("get-quote").removeHeader("*").setHeader(Exchange.HTTP_METHOD, constant("POST"))
                ...
                .setHeader("Content-Type", constant("application/json")).setBody(exchange -> exchange.getProperty(QUOTE_REQUEST_BODY))
                .marshal().json(JsonLibrary.Jackson).log(LoggingLevel.INFO, "Quote Request Body: ${body}")
