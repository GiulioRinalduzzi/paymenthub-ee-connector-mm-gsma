...
public class TransferResponseProcessor implements Processor {
    ...
    public void process(Exchange exchange) {
        ...
        if (hasTransferFailed != null && (boolean) hasTransferFailed) {
            ...
            zeebeClient.newPublishMessageCommand().messageName(TRANSFER_MESSAGE)
                    .correlationKey(exchange.getProperty(TRANSACTION_ID, String.class)).variables(variables).send().join();
        }
