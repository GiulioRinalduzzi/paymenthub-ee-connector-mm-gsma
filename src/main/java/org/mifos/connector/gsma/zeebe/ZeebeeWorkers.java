...
public class ZeebeeWorkers {
    ...
    public void setupWorkers() {
        ...
        zeebeClient.newWorker().jobType("payeeProcess").handler((client, job) -> {
            logger.info("Job '{}' started from process '{}' with key {}", job.getType(), job.getBpmnProcessId(), job.getKey());

            Map<String, Object> variables = job.getVariablesAsMap();
            TransactionChannelRequestDTO channelRequest = objectMapper.readValue((String) variables.get("channelRequest"),
        ...
        zeebeClient.newWorker().jobType("sendTimeoutChannel").handler((client, job) -> {
            ...
            client.newCompleteCommand(job.getKey()).send().join();
        }).name("sendTimeoutChannel").maxJobsActive(workerMaxJobs).open();

        zeebeClient.newWorker().jobType("sendFailureChannel").handler((client, job) -> {
            logger.info("Job '{}' started from process '{}' with key {}", job.getType(), job.getBpmnProcessId(), job.getKey());
            logger.info("TRANSACTION FAILED");
            client.newCompleteCommand(job.getKey()).send().join();
        }).name("sendFailureChannel").maxJobsActive(workerMaxJobs).open();

        zeebeClient.newWorker().jobType("sendConfirmationChannel").handler((client, job) -> {
            logger.info("Job '{}' started from process '{}' with key {}", job.getType(), job.getBpmnProcessId(), job.getKey());
            logger.info("TRANSACTION WAS SUCCESSFUL.");
            client.newCompleteCommand(job.getKey()).send().join();
        }).name("sendConfirmationChannel").maxJobsActive(workerMaxJobs).open();
